...
dict_parameters = {
    "chua": {"a": [15.6, 15.6], "b": [25, 51], "mu0": [-1.143, -1.143], "mu1": [-0.714, -0.714]},
...
class Attractor_Generator(object):
    ...
    def compute_attractor(self, attractor, x0=0, y0=0, z0=0, dt=0.01, num_steps=100000, var_params=False):
        ...
        if attractor in self.available_attractors_2d.keys():
            ...
            self.coordinates, self.t = compute_attractor_3d(attr_fn, x0, y0, z0, dt, num_steps, var_params)
        elif attractor == "random_noise3d":
            self.attractor_dimension = "3d"
            self.coordinates = np.random.normal(scale=1 / np.sqrt(3), size=(num_steps + 1, 3))
    ...
    def generate_data_file(self, path, filename, format="csv"):
        ...
        if self.attractor_dimension == "2d":
            dict_data = {"t": self.t, "x": self.coordinates[:, 0], "y": self.coordinates[:, 1]}
...
def compute_attractor_3d(attractor, x0, y0, z0, dt, num_steps, var_params):
    ...
    if var_params:
        ...
        a = de.OdeSystem(
            rhs, y0=y_init, dense_output=True, t=(t0, ind_tfin), rtol=1e-9, atol=1e-9, constants=dict_parameters
        )
    else:
        a = de.OdeSystem(rhs, y0=y_init, dense_output=True, t=(t0, ind_tfin), rtol=1e-9, atol=1e-9)
...
def becker(t, state, beta1=1.0, beta2=0.84, rho=0.048, kappaprime=0.8525, nu=4.2164 * 10**-5):
    ...
    kappacr2 = (
        ...
        + np.sqrt(
            ...
            + (kappacr1 + rho * rho * (beta2 - 1)) * (kappacr1 + rho * rho * (beta2 - 1))
        )
    ) / (2 + 2 * rho)
