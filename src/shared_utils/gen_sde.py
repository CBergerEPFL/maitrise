...
def _gen_data(
    ...
    for ii in np.arange(Nx):
        X_obs[:, ii] = X_obs[:, ii] + epsilon_add * cn.powerlaw_psd_gaussian(beta_add, Nt)
...
def _gen_lorenz_stochastic(
    ...
    t = np.linspace(0, (sample * (length + discard)) * step, sample * (length + discard))
    Nt_simulation = len(t)
    ...
    for tt in range(Nt_simulation - 1):
        r = np.random.randn(3)
        X[tt + 1, 0] = X[tt, 0] + (sigma * (X[tt, 1] - X[tt, 0])) * step + epsilon_sde * np.sqrt(step) * r[0]
        X[tt + 1, 1] = X[tt, 1] + (X[tt, 0] * (rho - X[tt, 2]) - X[tt, 1]) * step + epsilon_sde * np.sqrt(step) * r[1]
        X[tt + 1, 2] = X[tt, 2] + (X[tt, 0] * X[tt, 1] - beta * X[tt, 2]) * step + epsilon_sde * np.sqrt(step) * r[2]
...
def _gen_rossler_stochastic(
    length=10000, x0=None, a=0.2, b=0.2, c=5.7, step=0.001, sample=0.03, discard=1000, epsilon_sde=0.0, observables=[0]
):
