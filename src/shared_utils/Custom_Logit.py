...
class Logit_binary(LogisticRegression):
    def __init__(self,HR_index: int, penalty="l2", *, dual=False, tol=0.0001, C=1, fit_intercept=True, intercept_scaling=1, class_weight=None, random_state=None, solver="lbfgs", max_iter=100, multi_class="auto", verbose=0, warm_start=False, n_jobs=None, l1_ratio=None):
    ...
    def __TransformData(self,X,y = None):
        ...
        if ((HR_metrics==0) | (HR_metrics==1)).all():
            if y is None:
                ...
                X_HR = X[~cond,:]
                return X_HR,np.empty(len(index_arr_HR)),index_arr_noHR[0],index_arr_HR[0]
            else :
                cond = HR_metrics == 0
                index_arr_noHR = (X[:,self.index] == 0).nonzero()
                index_arr_HR = (X[:,self.index] == 1).nonzero()
            ...
            raise ValueError("The array associated with HR feature must have binary values (0/1)")

    def fit(self,X,y,sample_weight = None):
        if X.shape[0] != y.shape[0]:
            raise ValueError("Number of rows of input data matrix different than labels array size")
