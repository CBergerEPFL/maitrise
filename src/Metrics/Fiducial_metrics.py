...
def Corr_lead_score(signals,fs):
    ...
    for j in range(signals.shape[0]):
        val = (np.mean(np.abs(M[j,:]))) ##Why abs val ==> We want the correlation between lead. Two lead can be anti correlated because it measures the same event but from a different heart axis
        if np.isnan(val):
            val = 0
        results[j] = val
    return results
