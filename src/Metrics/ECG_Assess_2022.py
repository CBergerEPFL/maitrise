...
def stationary_signal_check(data):
    ...
    for lead in range(1,data.shape[0]+1):
        window_matrix = np.lib.stride_tricks.sliding_window_view(data[lead-1,:], window_length)[::10]
...
def signal_to_noise_ratio_check(data):
    res = []
    for lead in range(data.shape[0]):
        _, pxx_den = scipy.signal.periodogram(data[lead,:], fs=sampling_frequency, scaling="spectrum")
        if sum(pxx_den):
...
def processing(ECG, temp_freq):
    ...
    SQM = np.vstack([stationary_signal_check(ECG),heart_rate_check(filt_ECG),signal_to_noise_ratio_check(ECG)])
    res = np.array([])
