    ...
    for count, name in enumerate(feat_SQA):
        if name == "HR":
            ...
            X_test[count] = np.mean(
                dict_functions[name](signals, fs, normalization=True)
            )
...
def SQA_wrong_estimate(signals, fs, name_lead, y_label, Topt, interval):
    ...
    for final in range(signals.shape[0]):
        val = (
            5.2396 * arr_SNR[final] - 10.0932 * arr_TSD[final] + 6.6979 * arr_CC[final]
        )
        ...
        plt.figtext(
            1, 0.6, "Normalized TSD value value = {0:.2f}".format(arr_TSD[final])
        )
        plt.figtext(1, 0.5, "Intercorrelation value = {0:.2f}".format(arr_CC[final]))
        plt.figtext(
            1, 0.4, "Logistic Regression prediciton value = {0:.2f}".format(val)
        )
...
def SQANTSD_wrong_estimate(signals, fs, name_lead, y_label, Topt, interval):
    ...
    for final in range(signals.shape[0]):
        ...
        plt.figtext(
            1, 0.5, "Logistic Regression prediciton value = {0:.2f}".format(val)
        )
